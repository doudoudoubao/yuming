...
HELP_SECTIONS["命令"] = """🌐 <b>域名监控</b> · 使用说明 1/3

盯住你想要的域名，一旦释放立刻下单。

<b>最常用</b>
把域名直接发给我就能加入监控，不用打命令。
一次发多个也行，空格或换行分隔。
...
HELP_SECTIONS["模式"] = """📐 使用说明 2/3 · <b>批量写法</b>

<b>花括号</b>
<code>mydream.{com,net,io}</code>　一个名字，三个后缀
<code>{short,tiny}.com</code>　两个名字
<code>{vps,host}.{com,io}</code>　四种组合

<b>后缀合集</b>
<code>vps.{@all}</code>　全部 58 个无限制后缀
<code>vps.{@two}</code>　只要 33 个两位后缀
<code>vps.{@gtld}</code>　只要 25 个通用后缀
<code>vps.{@two,com}</code>　合集与具体后缀混写

另有两组<b>有注册门槛</b>的，特意不含在 @all 里：
<code>@europe</code>　多数要当地实体或居民身份
<code>@china</code>　.cn 需要实名认证

发 /tlds 看全部，/tlds all 看某组的完整内容。

<b>抢到一个就收工</b>
同一个名字展开出的域名算一组。抢到其中任意一个之后，
其余的自动停止监控并通知你。
「这个名字我要，哪个后缀都行」就该这么用。

<b>留意数量</b>
<code>vps.{@all}</code> 就是 58 个域名，每个都要轮询。
一条模式最多展开 200 个，超了会拦下。"""
...
<b>说句实话</b>
值钱的域名在释放那一刻会被专业抢注商拿走，
他们握着几十上百个注册商通道，我们抢不过。
本程序适合<b>没人跟你抢</b>的域名：小众名字、
个人项目名、别人忘了续费的 —— 这类占绝大多数。

<b>下单是在哪儿完成的</b>
本程序自己不卖域名，是调用<b>你自己的注册商账号</b>的 API 下单。
所以要先去 NameSilo / Dynadot / 阿里云 / GoDaddy / Namecheap
其中一家开户、充值、开 API，再把凭据填到服务器上。
服务器上执行 <code>domain-monitor registrar</code> 可看各家要填什么。
/status 里的「通道」就是当前配的注册商。

<b>/buy 手动下单</b>
需要已开启抢注功能，否则会拒绝。
不在监控列表里的域名会先自动加进去。"""

HELP_SECTIONS["状态"] = """🔍 <b>状态含义与推送</b>
...
通用后缀的标准流程：到期后 45 天续费宽限期，
30 天赎回期，5 天待删除，然后释放。
❔ 未知＝还没查过　⚠️ 查询失败＝会自动重试

<b>什么时候查得勤</b>
🌙 常规　6 小时一次
👀 盯紧　30 分钟一次（已进入删除流程）
⏱ 临近　1 分钟一次
🔥 冲刺　最后 5 分钟改用 DNS 高频探测

冲刺只为<b>真正会下单</b>的域名开。只通知不买的
（/auto 关掉的、仅监控模式、已暂停的）最多走到「临近」档，
不会做高频探测——那是为抢跑准备的，不抢就没必要。

预计释放时间从「待删除」起点推算，误差在小时级。
...
🧹 同组已抢到，其余停止监控
⚠️ 疑似误报、预算拦截、通道停用
状态变化（可注册和待删除会震手机，其余静音）

<b>误报是怎么防的</b>
一个还在正常注册期的域名突然查不到，
更可能是服务器抽风。这种可疑跳变会复核一次再当真。
走完删除流程掉出来的属于预期内，不复核、立刻抢。"""

# /help 默认发前三条；「状态」作为补充主题按需查看
HELP_ORDER = ["命令", "模式", "抢注"]
